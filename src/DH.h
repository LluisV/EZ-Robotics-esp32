...
/**
 * @brief Robot type enumeration for analytical IK
 */
enum class RobotType {
    GENERIC,     // Use numerical IK
    SCARA,       // SCARA robot
    CARTESIAN,   // Cartesian robot
    ARTICULATED_2R  // 2R planar robot
};
...
public:
     ...
     * @param robotType Type of robot for analytical solutions (default: GENERIC)
     */
    DH(int numberOfJoints, RobotType robotType = RobotType::GENERIC);
    ...
    /**
     * @brief Get the pose of the end-effector using quaternion for orientation.
     * 
     * @return Eigen::VectorXf A vector containing position (x,y,z) and quaternion (qw,qx,qy,qz).
     */
    Eigen::VectorXf getEndEffectorPoseQuaternion() const;
    ...
    /**
     * @brief Compute the manipulability measure at current configuration.
     * 
     * @return float Manipulability measure (0 = singular, higher = better).
     */
    float computeManipulability() const;

    /**
     * @brief Check if the robot is near a singularity.
     * 
     * @param threshold Threshold for singularity detection (default: 0.01).
     * @return bool True if near singularity, false otherwise.
     */
    bool isNearSingularity(float threshold = 0.01f) const;

    /**
     * @brief Check if a target position is within the robot's workspace.
     * 
     * @param position Target position (x,y,z).
     * @return bool True if position is reachable, false otherwise.
     */
    bool isPositionReachable(const Eigen::Vector3f& position) const;

    /**
     * @brief Get the maximum reach of the robot.
     * 
     * @return float Maximum reach distance from base.
     */
    float getMaxReach() const;

    /**
     * @brief Get the minimum reach of the robot.
     * 
     * @return float Minimum reach distance from base.
     */
    float getMinReach() const;

    /**
     * @brief Set the robot type for analytical IK solutions.
     * 
     * @param type Robot type.
     */
    void setRobotType(RobotType type) { robotType = type; }

    /**
     * @brief Get the robot type.
     * 
     * @return RobotType Current robot type.
     */
    RobotType getRobotType() const { return robotType; }

    /**
     * @brief Enable/disable transformation caching for performance.
     * 
     * @param enable True to enable caching.
     */
    void enableTransformCaching(bool enable);

    /**
     * @brief Invalidate cached transformations (call after joint changes).
     */
...
private:
    ...
    RobotType robotType;        // Type of robot for analytical solutions
    
    // Caching for performance optimization
    bool cachingEnabled;
    mutable bool cacheValid;
    mutable std::vector<Eigen::Matrix4f> cachedTransforms;
    mutable Eigen::Matrix4f cachedEndEffectorTransform;
    
    /**
     * @brief Update cached transformations.
     */
    void updateCache() const;
    
    /**
     * @brief Get cached or compute joint transform.
     */
    Eigen::Matrix4f getCachedJointTransform(int jointIndex) const;
};
