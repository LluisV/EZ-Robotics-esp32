/**
 * @file JobManager.h
 * @brief Job manager for running G-code files, optimized for smooth motor operation
 */
 ...
 /**
  * @brief Job manager class for running G-code files, optimized to minimize impact on motor performance
  */
 class JobManager
 {
 public:
   ...
   /**
    * @brief Perform maintenance tasks during idle time
    * This method should be called from the main loop during periods when motors aren't busy
    */
   ...
   /**
    * @brief Set throttle interval for file loading
    * @param interval Minimum time in ms between file load attempts
    */
   void setLoadThrottleInterval(unsigned long interval)
   {
     loadThrottleInterval = max(10UL, interval);
   }
 ...
 private:
   ...
   bool needsStateSave = false;    ///< Flag to indicate job state should be saved
 
   unsigned long lastLoadAttemptTime;  ///< Time of last load attempt (ms)
   unsigned long loadThrottleInterval; ///< Minimum time between load attempts (ms)
   ...
   // Pre-allocated string to avoid memory allocations during G-code processing
   String commandBuffer;
   
   // Pre-allocated vector to collect valid commands before sending to queue
   std::vector<String> validCommandBuffer;
