 ...
 */
 ...
 String GRBLCommunicationManager::composeStatusReport()
 {
     ...
     // Get machine state
     if (machineController) {
         // Determine machine status based on motion state
         if (machineController->isMoving()) {
             machineStatus = "Run";
             if (feedHoldActive) {
                 machineStatus = "Hold";
             }
         ...
         std::vector<float> worldPos = machineController->getCurrentWorldPosition();
         std::vector<float> workPos = machineController->getCurrentWorkPosition();
         ...
         // Get feedrate
         feedRate = String(machineController->getCurrentFeedrate(), 0);
     }
     ...
     if (machineController && machineController->getMotionPlanner()) {
